...
// Tradução para exibir os dias da semana na interface
const DAYS_MAP = {
  'domingo': 'Dom',
  'segunda': 'Seg',
  'terça': 'Ter',
...
// Mapeamento para converter de inglês para português (usado em dados existentes)
const DAY_TRANSLATION_MAP = {
  'sunday': 'domingo',
  'monday': 'segunda',
  'tuesday': 'terça',
  'wednesday': 'quarta',
...
const RoutinesPage = () => {
  ...
  // Função para converter dias em inglês para português (para dados existentes)
  const translateDays = (days: string[] | undefined): string[] => {
    if (!days) return [];
    return days.map(day => DAY_TRANSLATION_MAP[day as keyof typeof DAY_TRANSLATION_MAP] || day);
  };
  ...
  const handleEdit = (routine: RoutineActivity) => {
    // Converter dias em inglês para português ao editar
    const translatedDays = translateDays(routine.days);
  ...
  // Formatar horário para exibição no padrão brasileiro
  const formatTime = (time: string): string => {
    return time; // O formato HH:MM já é adequado para o Brasil
  };
        ...
        <div className="bg-white shadow-sm rounded-lg overflow-hidden">
          <ul className="divide-y divide-gray-200">
            {sortedRoutines.map(routine => {
              // Traduzir os dias para exibição
              const translatedDays = translateDays(routine.days);
                      ...
                      <div className="ml-4">
                        ...
                        <div className="flex items-center text-sm text-gray-500">
                          ...
                          {translatedDays.length > 0 && (
                            <span className="ml-4">
                              {translatedDays.map(day => DAYS_MAP[day as keyof typeof DAYS_MAP] || day).join(', ')}
                            </span>
                          )}
                        </div>
                      </div>
